...
//
// 6/12/2019 : calcul de l'erreur moyenne et application du Trainer sur les jeux de test
...
void TrainSimpleFeedForwardClassifier(const DeviceDescriptor& device)   // CPU
{
    ...
    double test_result = 0;                                             // Calcul de l'erreur moyenne
    for (size_t i = 0; i < numMinibatchesToTrain; ++i)
    {
        ...
        test_result += trainer->PreviousMinibatchEvaluationAverage();   // Calcul de l'erreur moyenne
    ...
    printf("Average test error = %.2f %%\n", test_result * 100 / numMinibatchesToTrain);
}

void TrainMNISTClassifier(const std::wstring sourceFile, const DeviceDescriptor& device)   // GPU
{
    ...
    const size_t numSweepsToTrainWith = 10;
    const size_t numMinibatchesToTrain = (numSamplesPerSweep * numSweepsToTrainWith) / minibatchSize;

    auto featureStreamName = L"features";
    auto labelsStreamName = L"labels";
    auto minibatchSource = TextFormatMinibatchSource(sourceFile, { { featureStreamName, inputDim }, { labelsStreamName, numOutputClasses } });
...
void TrainMNISTSeqClassifier(const std::wstring sourceFile, const DeviceDescriptor& device)    // GPU
{
    ...
    auto featureStreamName = L"features";
    auto labelsStreamName = L"labels";
    auto minibatchSource = TextFormatMinibatchSource(sourceFile, {{featureStreamName, inputDim * inputDim}, {labelsStreamName, numOutputClasses}});
...
void MNISTClassifierTests()
{
    ...
    if (ShouldRunOnGpu())
    {
        ...
        TrainMNISTClassifier(L"Train-28x28_cntk_text.txt", DeviceDescriptor::GPUDevice(0));

        fprintf(stderr, "\nTest MNIST Classifier...\n");
        TrainMNISTClassifier(L"Test-28x28_cntk_text.txt", DeviceDescriptor::GPUDevice(0));

        // Evaluation / test
        fprintf(stderr, "\nTrain MNIST Seq Classifier...\n");
        TrainMNISTSeqClassifier(L"Train-28x28_cntk_text.txt",DeviceDescriptor::GPUDevice(0));

        fprintf(stderr, "\nTest MNIST Seq Classifier...\n");
        TrainMNISTSeqClassifier(L"Test-28x28_cntk_text.txt", DeviceDescriptor::GPUDevice(0));
